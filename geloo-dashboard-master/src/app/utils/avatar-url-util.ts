...
export const AVATAAARS_OPTIONS = {
  ...
  eyes: [
    'default', 'closed', 'cry', 'eyeRoll', 'happy', 'hearts', 'side', 'squint', 'surprised', 'wink', 'winkWacky', 'xDizzy'
  ],
  eyebrows: [
    'default', 'angry', 'angryNatural', 'defaultNatural', 'flatNatural', 'frownNatural', 'raisedExcited', 'raisedExcitedNatural', 'sadConcerned', 'sadConcernedNatural', 'unibrowNatural', 'upDown', 'upDownNatural'
  ],
  mouth: [
    'default', 'concerned', 'disbelief', 'eating', 'grimace', 'sad', 'screamOpen', 'serious', 'smile', 'tongue', 'twinkle', 'vomit'
  ]
};
...
export class AvatarUrlUtil {
  ...
  static getAvatarUrl(profile: AvatarProfile, backendRoot: string = ''): string {
    if (!profile) return 'assets/img/default-avatar.png';
    ...
    if (profile.mainAvatar && !profile.mainAvatar.includes('dicebear.com')) {
      return profile.mainAvatar.startsWith('http') ? profile.mainAvatar : backendRoot + (profile.mainAvatar.startsWith('/') ? '' : '/') + profile.mainAvatar;
    }
    ...
    // 4. Last resort: Generate a default DiceBear avataaars URL (matching main app's getDefaultAvatar)
    const seed = profile._id || profile.id || 'default';
