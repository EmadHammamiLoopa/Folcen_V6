...
export class SettingsPage implements OnInit {
  ...
  togglePrivacy(event) {
    if (this.isPrivate === this.user.isPrivate) return;

    this.loading = true;
    this.userService.updatePrivacy(this.isPrivate).subscribe(
      (resp: any) => {
        this.loading = false;
        this.user.isPrivate = this.isPrivate;
        this.toastService.presentStdToastr(resp.message || 'Privacy settings updated');
        this.userService.setCurrentUser(this.user);
      },
      (err) => {
        this.loading = false;
        this.toastService.presentStdToastr(err.message || 'Error updating privacy settings');
      }
    );
  }
