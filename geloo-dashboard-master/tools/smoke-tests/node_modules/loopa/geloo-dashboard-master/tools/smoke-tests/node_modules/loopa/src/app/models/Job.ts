...
export class Job {
  ...
  get isVerified(): boolean {
    if (!this.email || !this.website) return false;
    
    const freeEmailDomains = ['gmail.com', 'yahoo.com', 'outlook.com', 'hotmail.com', 'icloud.com', 'aol.com'];
    const emailDomain = this.email.split('@')[1]?.toLowerCase();
    
    if (!emailDomain || freeEmailDomains.includes(emailDomain)) return false;
    
    // Basic website check (must have a dot and be at least 4 chars)
    if (!this.website || this.website.length < 4 || !this.website.includes('.')) return false;
  ...
  constructor(job: Job) {
    ...
    this.workingHours = job.workingHours;
  }
  ...
  get workingHours(): string { return this._workingHours }
  ...
  set photos(photos: string[]) {
    this._photos = photos.map(photo => (!photo.includes(constants.DOMAIN_URL) ? constants.DOMAIN_URL : '') + photo);
  }
  ...
  set skills(skills: string) { this._skills = skills }
  set workingHours(workingHours: string) { this._workingHours = workingHours }
}
