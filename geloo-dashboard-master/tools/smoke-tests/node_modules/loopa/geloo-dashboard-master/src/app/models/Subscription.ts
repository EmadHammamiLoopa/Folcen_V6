export class Subscription {
  ...
  constructor(subscription: Partial<Subscription>, user?: { id: string; firstName: string; lastName: string }) {
    ...
    this._dayPrice = subscription._dayPrice || 0;
    this._weekPrice = subscription._weekPrice || 0;
    this._monthPrice = subscription._monthPrice || 0;
    this._yearPrice = subscription._yearPrice || 0;
    this._currency = subscription._currency || 'USD';
    this._createdAt = subscription._createdAt ? new Date(subscription._createdAt) : new Date();
    this._updatedAt = subscription._updatedAt ? new Date(subscription._updatedAt) : new Date();
