...
onCompanySelectionChange(value: string) {
  ...
  getServiceForm() {
    ...
    form.append('serviceCategory', this.form.get('serviceCategory').value);
    form.append('serviceRate', this.form.get('serviceRate').value);
    form.append('availability', this.form.get('availability').value);
    form.append('Experience', this.form.get('Experience').value);
    form.append('serviceDuration', this.form.get('serviceDuration').value);
    form.append('paymentMethods', JSON.stringify(this.form.get('paymentMethods').value));
    form.append('licenseCertification', this.form.get('licenseCertification').value);
    form.append('websitePortfolio', this.form.get('websitePortfolio').value);
    form.append('address', this.form.get('address').value);
    
    // Append all photos
    this.serviceImages.forEach((img, index) => {
      if (img.file) {
    ...
    if (this.serviceImages.length > 0) {
      form.append('photo', this.serviceImages[0].file, this.serviceImages[0].name);
    }
  ...
  async submit() {
    ...
    const modal = await this.modalController.create({
      component: ServiceDisclaimerComponent,
      cssClass: 'disclaimer-modal'
    });
    await modal.present();

    const { data } = await modal.onDidDismiss();
    if (!data) return; // User cancelled or didn't accept
  ...
  calculateCompletion() {
    ...
    let totalFields = Object.keys(controls).length;
    let filledFields = 0;
