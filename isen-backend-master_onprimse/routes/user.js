...
async function armPeerWakeTimeout(callerId, calleeId, meta = {}) {
  ...
  const remaining = Number(meta.expiresAt || 0) - Date.now();
  const delayMs = Number.isFinite(remaining) && remaining > 0
    ? remaining
    : callSessions.RING_TIMEOUT_MS;

  const timer = setTimeout(async () => {
    try {
      const activeCallId = callSessions.getActiveCallId(from, to);
      const state = callSessions.callStates.get(callId);
      ...
      if (String(activeCallId || '') !== callId || state?.state !== 'ringing') {
        return;
      }

      const at = Date.now();
      const lifecycleResult = await CallEvent.updateOne(
        {
          callId,
          'lifecycle.event': { $ne: 'timeout' }
        },
        {
          $push: {
            lifecycle: {
      ...
      if (changed !== 1) {
        console.log('[peer-wake][timeout] already recorded', { callId });
        return;
      }

      const updatedUser = await User.findByIdAndUpdate(
        to,
        { $inc: { missedCallBudget: 1 } },
        { new: true }
      ).select('missedCallBudget').lean();
      ...
      console.log('[peer-wake][timeout] MISSED RECORDED', {
        callId,
        to,
        missedCallBudget: updatedUser?.missedCallBudget
      });

      emitToUser(to, 'budget-update', {
        missedCallBudget: updatedUser?.missedCallBudget
      });
      emitToUser(to, 'video-call-missed-timeout', payload);
      emitToUser(to, 'missed-call', payload);
      emitToUser(to, 'video-call-timeout', { ...payload, notify: true });
      emitToUser(from, 'video-call-timeout', { ...payload, notify: false });
      emitToUser(to, 'video-canceled', { ...payload, notify: true });
      emitToUser(from, 'video-canceled', { ...payload, notify: false });
      ...
      sendVideoCallLifecyclePush(
        ...
        'video_call_timeout',
        {
          ...
          reason: 'timeout',
          timestamp: at,
          expiresAt: meta.expiresAt || at
        }
      ).catch(err =>
        console.warn('[peer-wake][timeout] FCM failed', err?.message || err)
      );

      callSessions.setCallState(
        callId,
        'timeout',
        { from, to, reason: 'timeout' }
      );
      callSessions.clearRingTimer(from, to);
      callSessions.clearActivePair(from, to);
    } catch (err) {
      console.error('[peer-wake][timeout] failed', err);
    }
  }, delayMs);

  callSessions.ringTimers.set(callSessions.keyOf(from, to), timer);
  console.log('[peer-wake] authoritative timeout armed', { callId, delayMs });
}
...
router.get('/:userId/peer', [requireSignin, withAuthUser], async (req, res, next) => {
  try {
    ...
    // A non-friend may call only when that caller previously requested access
    // from this exact callee and the callee accepted it. The accepted chat
    // request is the persistent, directional permission record.
    ...
    if (req.query?.wake === '1' || req.query?.wake === 'true') {
      ...
      await armPeerWakeTimeout(callerId, userId, callInvite);
...
router.get('/retention', [requireSignin, isAdmin], AdminController.getRetention);
      ...
      if (typeof obj === 'object') {
        ...
        for (const key in obj) {
          ...
          if ((key === 'interests' || key === 'languages')) {
            const decode = (raw) => {
              if (!raw) return raw;
              if (Array.isArray(raw) && raw.length === 1 && typeof raw[0] === 'string') raw = raw[0];
